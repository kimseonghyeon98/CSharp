...
namespace A124_LinqGroupBy
{
    ...
    class Program
    {
        static List<Student> students; static void Main(string[] args)
        {
            ...
            {
                Console.WriteLine("{0} : 학생  수  = {1}", item.key, item.count); Console.WriteLine("{0} : 평균  점수  = {1:F2}", item.key, item.avr); Console.WriteLine("{0} : 최고  점수  = {1:F2}", item.key, item.max); Console.WriteLine();
            }
        }
    }
}